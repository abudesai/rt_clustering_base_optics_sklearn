...
def tune_hyperparameters(data, data_schema, num_trials, hyper_param_path, hpt_results_path):  
    ...
    preprocessed_data, _  = model_trainer.preprocess_data(data, data_schema)       
    ...
    def objective(**hyperparameters):
        ...
        # train model
        model, pred_clusters = model_trainer.train_model(train_X, hyperparameters) 
        
        # evaluate the model
        score = model_trainer.evaluate_clusters(train_X, pred_clusters)
        # Our optimizing metric is the model loss fn
        opt_metric = np.round(score, 5)   # returns dbi
